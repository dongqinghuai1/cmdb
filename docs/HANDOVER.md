...
## 33. 里程碑 M2026-09-18b：值班排班 DutySchedule API（system 域 V1.1#23 有表无 API 补齐）

- **背景**：`system_dutyschedule` 表在 ER V1.1#23 早已建表（主/备班 + 交班字段），但始终无 API/页面载体——"固件升级/值班"的用户点名项先落值班一半。
- **端点** `/api/v1/system/duty-schedules/`（`DutyScheduleViewSet`，router 注册）：CRUD（同人同日同班次由 ModelSerializer UniqueTogetherValidator 拦 400）；`GET .../calendar/?month=YYYY-MM` 月聚合视图（31 天，每天 primary/backup 班位含 user_id/user_name/region/handover_note/handed_off_at，同槽多行按 created_at 最新占位、`all` 留全量——防覆盖）；`POST /{id}/handoff/` 交班（置 handed_off_at=now + 备注 + 审计）。写（create/update/delete/handoff）走 `perform_*` + `_need_edit` 二次门禁。
...
## 38. 里程碑 M2026-09-22b：PDU 电源实测（dcim 电源域 + SNMP/Prometheus 双适配）

- **范围**：PDU/UPS 在台账上本就是 cmdb 设备（init_nops_data 预置 model.code=pdu/ups，facility），且 Device 已带 `rated_power_w` 额定功率。本轮补"实测样本 + 采集适配 + 总览"，不新增任何并行采集器：样本写入只发生在轮询动作/周期任务内，通道复用既有 SNMP（apps/cmdb/snmp.py）与 Prometheus（apps/cmdb/prometheus.py）只读消费。
- **模型**（dcim.0004，仅新增 PowerSample）：`device_id`(裸 FK)+`outlet`(输出口，'' 总路)+watts/current_a/voltage_v+`utilization_pct`+`rated_watts`(采样时刻额定快照)+source(prom/snmp/manual)+sampled_at；同刻多输出口取末行(-id tie-break)。
- **采集适配**（apps/cmdb/snmp.py `collect_pdu` + apps/dcim/power.py）：
  - Prometheus 主通道：`poll_prom()` 复用 prometheus.query_once，环境 `NOPS_PROM_POWER_QUERIES` 模板驱动（未配置 skipped，与 prom_poll 降级语义一致）；
  - SNMP 通道：`poll_snmp(mock=)` — mock 演练（2 输出口样例落库）；真实模式 PDU-MIB 供应商模板未校准主动 raise（沿用无线 RequiresCalibration 纪律，不写假数据）；
  - 周期任务 `dcim.poll_power`（beat 5 分钟）跑 Prom 通道；样本 30 天滚动清理。
- **端点** `/dcim/power-samples/`：历史只读（集合 POST 405，样本只能由采集链路写入）；`POST poll`{mock/source/device_ids}（dcim.power.edit 门禁）；`GET summary`（每台最近实测+总功率+≥80% 超阈值计数）。权限点 dcim.power.view/edit 入 init_nops_data（net_ops 正则已含 dcim；net_admin=view、dcim_admin=view+edit、sys_admin 无、auditor 无）。cmdb purge 联动清除电源样本。
- **验证**：`scripts/verify_power.py` **15 PASS ×2**（sqlite/PG）：纯函数 P1-P2 → 建 2 台 PDU(额定) → 权限读门 S2-S4 → mock 轮询写样本 S5-S6 → 每台 2 条历史 S7-S8 → 汇总(利用率 82.7%/103.3%、总功率 4960W、超阈值 2) S9-S11 → 改额定 3000→3500 后重采样利用率回落 70.9% S12 → purge 联动清理 S13。回归 api_test 33、verify_baseline 15（PG）保持绿。
- **待办/坑**：① PDU-MIB 供应商模板（Raritan/APC…）校准后即可开真实 SNMP 直采（seam 已留，mock=False 即走真网）；② Prometheus 模板变量文档化（outlet_label/device_field 语义在 NOPS_PROM_POWER_QUERIES）与真实机房样例待补；③ 前端"机柜供电"卡片（利用率为 0-80-100 色阶、超阈值告警联动 alert.rule）待做，后端已就绪。

---

## 39. 里程碑 M2026-09-23：vCenter 虚机同步（cmdb 同步源 + 幂等 upsert + 收敛软删/复活）

- **范围**：把"虚机 vCenter 同步"接到台账 —— 新增同步源实体（vCenter 连接参数/名单/最近结果），虚机落库为**既有 cmdb.Device**（is_virtual=True、vm_source='vcenter:<源pk>'、vm_uuid 幂等键、attrs 存 cpu/内存/系统/电源态/宿主机/集群/数据中心、电源态映射 online_status），模型 code='vm' 由 init 预置直接复用。
- **模型**（cmdb.0006_vmwaresource）：`VmwareSource`{name 唯一、host、username、secret(EncryptedTextField 加密落库、序列化 write_only 不回显)、site_id/region_id(裸 FK)、is_active、mock_vms(收敛演练名单)、last_sync_at/last_result}。
- **采集适配**（apps/cmdb/vcenter.py + vmsync.py）：拉取层分层同 snmp.py —— mock 内置样例（以源名为前缀 web-01/db-01 两台）；真实 pyVmomi 依赖/连接模板未就绪前 raise RequiresCalibration（**不写假数据**）。`run_sync`：存活集过滤 deleted_at→upsert（attrs/电源态/标识有差异才写）；收敛=不在本次清单的同源虚机软删；软删者重新出现**复活**（deleted_at 置空，不重复建档）。周期任务 `cmdb.sync_vcenter`（beat 每小时 25 分）逐启用源 mock=False 拉取，校准源记录不中断。
...
## 37. 里程碑 M2026-09-22：报表中心 v1（report 域从零落地）

- **范围**：`apps/report` 原为空骨架（只有 ER ch.4 注释）。本轮落地"指标快照 + 订阅推送"模型：不新起采集/统计代理，全部在请求/周期任务内对各 app 既有表做只读聚合，sqlite/PG 双通。
- **模型**（`report.0001_initial`）：`ReportSnapshot`（report_type=inventory/alerts/changes/ncm、period_start/end、content JSON、built_by manual/auto、duration_ms、remark；(type, period_start) 唯一 → 同日幂等覆盖）；`ReportSchedule`（name 唯一、report_type、hour 展示位、enabled、notify_channel_ids/receivers、last_run_at、created_by_id）。
- **服务/聚合**（`report/services.py`）：`build_snapshot/save_snapshot` + 四类 builder——inventory(总数/在线离线/厂商/站点/今日新增)、alerts(状态/级别分布、周期新增与恢复、根因抑制中计数、Top 规则)、changes(变更/事件单按状态 + 周期新增)、ncm(备份覆盖设备数、周期备份数、基线最近 500 项合规率、配置变更事件数)，每类带一句 summary（推送文案）。周期任务 `report.daily_snapshot`（beat 每日 06:45）遍历启用订阅生成 + best-effort 推通知渠道。
- **端点** `/api/v1/reports/`：snapshots 只读列表（report_type/built_by/period_after 过滤）+ `POST generate`（report.snapshot.edit 门禁，幂等）+ `GET latest/`（每类最近一份，看板用）；schedules CRUD（edit 门禁）+ `POST {id}/run/` 立即生成 + `GET overview/`（订阅数与每类最近快照时间）。权限点 report.snapshot.view/edit 加入 init_nops_data（net_ops 正则含 report、net_admin=view、sys_admin=view+edit；auditor 不可见）。
- **验证**：`scripts/verify_report.py` **21 PASS ×2**（sqlite/PG）：读门 R1-R3 → 四类型生成 → 同日重复生成幂等同 id → latest 每类一份 → net/auditor 生成 403、非法类型 400 → 订阅建/重名 400/停用/立即 run(last_run+summary)/net run 403/overview/删除 → 幂等份数断言。回归 api_test 33、verify_alert_converge 18、verify_suppress 19（PG）保持绿。
- **待办/坑**：① 订阅粒度仅"每日 + hour 展示"，周/月聚合与"窗口内不发重复"待扩展；② receivers 站内推送未接（系统通知体系欠账）；③ 快照内容不含容量（机柜/端口利用率）与成本，如需加类型即可扩展 builder 注册表；④ 报表前端页（看板卡片/趋势图）待做，后端端点已就绪。
